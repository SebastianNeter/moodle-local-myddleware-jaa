...
| `username` / `email` | string \| null | Identity fields used by the app **only** to pair a person's pre and post responses. **The app hashes both at sync time and never stores them raw** — see the anonymity cross-cutting rule in spec.md. **Null when `anonymous` is `true`** |
